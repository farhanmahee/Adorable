...
@objc
public class AdorableDevLauncherUrl: NSObject {
  @objc
  public var url: URL
  ...
  public init(_ url: URL) {
    self.queryParams = AdorableDevLauncherURLHelper.getQueryParamsForUrl(url)

    if AdorableDevLauncherURLHelper.isDevLauncherURL(url),
      let urlParam = queryParams["url"],
      let urlFromParam = URL(string: urlParam) {
      self.url = AdorableDevLauncherURLHelper.replaceEXPScheme(urlFromParam, to: "http")
    } else {
      self.url = AdorableDevLauncherURLHelper.replaceEXPScheme(url, to: "http")
    }
...
@objc
public class AdorableDevLauncherURLHelper: NSObject {
  @objc
