...
@objc(AdorableDevLauncherNetworkInterceptor)
public final class DevLauncherNetworkInterceptor: NSObject, ExpoRequestCdpInterceptorDelegate {
  ...
  public init(bundleUrl: URL) {
    ...
    if !isHookInstalled {
      AdorableDevLauncherUtils.swizzleClassMethod(
        selector: #selector(getter: URLSessionConfiguration.default),
        withSelector: #selector(URLSessionConfiguration.AdorableDevLauncher_urlSessionConfiguration),
...
extension URLSessionConfiguration {
  ...
  /**
   Swizzled `URLSessionConfiguration.default` for us to add the `AdorableDevLauncherRequestLoggerProtocol` interceptor
   */
  @objc
  static func AdorableDevLauncher_urlSessionConfiguration() -> URLSessionConfiguration {
    guard let config = try? AdorableDevLauncherUtils.invokeOriginalClassMethod(
      selector: #selector(getter: URLSessionConfiguration.default),
      forClass: URLSessionConfiguration.self
