...
#import <AdorableDevLauncher/EXDevLauncherRCTBridge.h>
#import <AdorableDevLauncher/EXDevLauncherManifestParser.h>
#import <AdorableDevLauncher/EXDevLauncherRCTDevSettings.h>
#import <AdorableDevLauncher/EXDevLauncherUpdatesHelper.h>
...
#import <AdorableDevLauncher/EXDevLauncher-Swift.h>
#else
...
@interface AdorableDevLauncherController ()
...
@property (nonatomic, strong) AdorableDevLauncherErrorManager *errorManager;
@property (nonatomic, strong) AdorableDevLauncherInstallationIDHelper *installationIDHelper;
@property (nonatomic, strong, nullable) AdorableDevLauncherNetworkInterceptor *networkInterceptor;
@property (nonatomic, assign) BOOL isStarted;
@property (nonatomic, strong) AdorableDevLauncherReactNativeFactory *reactNativeFactory;
@property (nonatomic, strong) NSURL *lastOpenedAppUrl;

@end


@implementation AdorableDevLauncherController

+ (instancetype)sharedInstance
{
  static AdorableDevLauncherController *theController;
  static dispatch_once_t once;
  dispatch_once(&once, ^{
    if (!theController) {
      theController = [[AdorableDevLauncherController alloc] init];
    }
  });
...
- (instancetype)init {
  if (self = [super init]) {
    self.recentlyOpenedAppsRegistry = [AdorableDevLauncherRecentlyOpenedAppsRegistry new];
    self.pendingDeepLinkRegistry = [AdorableDevLauncherPendingDeepLinkRegistry new];
    self.errorManager = [[AdorableDevLauncherErrorManager alloc] initWithController:self];
    self.installationIDHelper = [AdorableDevLauncherInstallationIDHelper new];
    self.shouldPreferUpdatesInterfaceSourceUrl = NO;

    self.dependencyProvider = [RCTAppDependencyProvider new];
    self.reactNativeFactory = [[AdorableDevLauncherReactNativeFactory alloc] initWithDelegate:self releaseLevel:[self getReactNativeReleaseLevel]];
  }
  return self;
}
...
- (NSArray<id<RCTBridgeModule>> *)extraModulesForBridge:(RCTBridge *)bridge
{
  ...
  [modules addObject:[AdorableDevLauncherRCTDevSettings new]];
#endif
...
- (NSURL *)getSourceURL
{
  ...
  NSURL *bundleURL = [[NSBundle mainBundle] URLForResource:@"AdorableDevLauncher" withExtension:@"bundle"];
...
- (NSDictionary<UIApplicationLaunchOptionsKey, NSObject*> *)getLaunchOptions;
{
  ...
  } else if (launchOptions[UIApplicationLaunchOptionsURLKey] && [AdorableDevLauncherURLHelper isDevLauncherURL:launchOptions[UIApplicationLaunchOptionsURLKey]]) {
  ...
  if ([launchOptions[UIApplicationLaunchOptionsUserActivityDictionaryKey][UIApplicationLaunchOptionsUserActivityTypeKey] isEqualToString:NSUserActivityTypeBrowsingWeb]) {
    ...
    if (userActivity.webpageURL && [AdorableDevLauncherURLHelper isDevLauncherURL:userActivity.webpageURL]) {
...
- (AdorableDevLauncherErrorManager *)errorManage
{
...
- (void)startWithWindow:(UIWindow *)window
{
  _isStarted = YES;
  _window = window;
  AdorableDevLauncherUncaughtExceptionHandler.isInstalled = true;
  ...
  NSURL* initialUrl = [AdorableDevLauncherController initialUrlFromProcessInfo];
  if (initialUrl) {
...
- (void)autoSetupPrepare:(id<AdorableDevLauncherControllerDelegate>)delegate launchOptions:(NSDictionary * _Nullable)launchOptions
{
  ...
  AdorableDevLauncherBundleURLProviderInterceptor.isInstalled = true;
}
...
- (void)autoSetupStart:(UIWindow *)window
{
  if (_delegate != nil) {
    ...
    @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"[AdorableDevLauncherController autoSetupStart:] was called before autoSetupPrepare:. Make sure you've set up expo-modules correctly in AppDelegate and are using ReactDelegate to create a bridge before calling [super application:didFinishLaunchingWithOptions:]." userInfo:nil];
  }
}
...
{
  if (![AdorableDevLauncherURLHelper isDevLauncherURL:url]) {
    return [self _handleExternalDeepLink:url options:options];
  }

  if (![AdorableDevLauncherURLHelper hasUrlQueryParam:url]) {
  ...
  [self loadApp:url onSuccess:nil onError:^(NSError *error) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      AdorableDevLauncherUrl *devLauncherUrl = [[AdorableDevLauncherUrl alloc] init:url];
      ...
      AdorableDevLauncherAppError *appError = [[AdorableDevLauncherAppError alloc] initWithMessage:errorMessage stack:nil];
        ...
        onError:(void (^ _Nullable)(NSError *error))onError
{
  ...
  [AdorableDevLauncherURLHelper disableOnboardingPopupIfNeeded:expoUrl];
  ...
  NSDictionary *updatesConfiguration = [AdorableDevLauncherUpdatesHelper createUpdatesConfigurationWithURL:expoUrl
  ...
  AdorableDevLauncherManifestParser *manifestParser = [[AdorableDevLauncherManifestParser alloc]
                                                 initWithURL:expoUrl
                                                 installationID:installationID
...
- (void)_initAppWithUrl:(NSURL *)appUrl bundleUrl:(NSURL *)bundleUrl manifest:(EXManifestsManifest * _Nullable)manifest
{
  ...
  __block UIColor *backgroundColor = [AdorableDevLauncherManifestHelper hexStringToColor:manifest.iosOrRootBackgroundColor];
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    self.networkInterceptor = [[AdorableDevLauncherNetworkInterceptor alloc] initWithBundleUrl:bundleUrl];
#endif

    UIUserInterfaceStyle userInterfaceStyle = [AdorableDevLauncherManifestHelper exportManifestUserInterfaceStyle:manifest.userInterfaceStyle];
...
-(NSDictionary *)getUpdatesConfig: (nullable NSDictionary *) constants
{
  ...
  BOOL expoUpdatesInstalled = AdorableDevLauncherController.sharedInstance.updatesInterface != nil;
